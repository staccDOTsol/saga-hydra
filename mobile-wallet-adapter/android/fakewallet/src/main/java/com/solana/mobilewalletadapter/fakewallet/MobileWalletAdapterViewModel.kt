...
class MobileWalletAdapterViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun signAndSendTransactionsSimulateSign(request: MobileWalletAdapterServiceRequest.SignAndSendTransactions) {
        viewModelScope.launch {
            ...
            val valid = signingResults.map { result -> anchor_lang::Result.signature.isNotEmpty() }
            if (valid.all { it }) {
                Log.d(TAG, "Simulating signing with ${request.request.publicKey}")
                val signatures = signingResults.map { result -> anchor_lang::Result.signature }
                val signedTransactions = signingResults.map { result -> anchor_lang::Result.signedPayload }
    ...
    private fun chainOrClusterToRpcUri(chainOrCluster: String?): Uri {
        return when (chainOrCluster) {
            ...
            ProtocolContract.CLUSTER_MAINNET_BETA ->
                Uri.parse("https://jarrett-solana-7ba9.mainnet.rpcpool.com/8d890735-edf2-4a75-af84-92f7c9e31718")
