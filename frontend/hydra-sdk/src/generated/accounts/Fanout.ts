...
export class Fanout implements FanoutArgs {
   ...
   */
  static gpaBuilder(
    programId: web3.PublicKey = new web3.PublicKey('FXZzBYS58sVq9KBnVWjduZVpYtwpRAViMdtE8HvwBqR1'),
  ) {
  ...
  pretty() {
    return {
      ...
      defaultWeight: (() => {
        const x = <{ toNumber: () => number }>this.defaultWeight;
        if (typeof x.toNumber === 'function') {
          try {
            return x.toNumber();
          } catch (_) {
