...
pub fn calculate_inflow_change(total_inflow: u64, last_inflow: u64) -> anchor_lang::Result<u64> {
...
pub fn update_fanout_for_add(fanout: &mut Account<Fanout>, shares: u64)-> anchor_lang::Result<()> {
    msg ! ("update_fanout_for_add");
    msg ! ("fanout.total_available_shares: {}", fanout.total_available_shares);
    msg ! ("shares: {}", shares);
    msg ! ("fanout.total_members: {}", fanout.total_members);
    msg ! ("fanout.total_shares: {}", fanout.total_shares);
    let mut less_shares = fanout
    .total_available_shares;
    if shares != fanout.default_weight {
        less_shares = fanout
        .total_available_shares
        .checked_sub(shares)
        .or_arith_error()?;
...
pub fn update_fanout_for_remove(fanout: &mut Account<Fanout>)-> anchor_lang::Result<()> {
...
pub fn update_inflow(fanout: &mut Fanout, current_snapshot: u64)-> anchor_lang::Result<()> {
    msg ! ("update_inflow");
    msg ! ("fanout.total_inflow: {}", fanout.total_inflow);
    msg ! ("current_snapshot: {}", current_snapshot);
    msg ! ("fanout.last_snapshot_amount: {}", fanout.last_snapshot_amount);
